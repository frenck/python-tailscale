...
class SshPolicy(AclBase):
    ...
    check_period: Optional[str] = Field(alias="checkPeriod") # for checking access, e.g. "12h"

class AclTest(BaseModel):
    """Object holding Tailscale ACL Test information."""
    src: str # user, group, tags, hosts
    accept: Optional[List[str]] # hosts, ips, tags, etc that src should be able to access
    deny: Optional[List[str]] # hosts, ips, tags, etc that src should not be able to access
...
class AuthKeyRequest(BaseModel):
    ...
    expirySeconds: int = Field(default=86400, alias="expiry_seconds")

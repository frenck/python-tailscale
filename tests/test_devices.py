...
@pytest.mark.asyncio
async def test_device_tag_update_no_tag_prefix(aresponses: ResponsesMockServer) -> None:
    """Test updating a devices tags."""
    aresponses.add(
        "api.tailscale.com",
        "/api/v2/device/test/tags",
        ...
        body_pattern='{"tags": ["tag:no-prefix"]}',
    )

    async with aiohttp.ClientSession() as session:
        tailscale = Tailscale(tailnet="frenck", api_key="abc", session=session)
        await tailscale.tag_device("test", ["no-prefix"])  # nothing returned
        assert (
            aresponses.history[0].request.headers["Content-Type"] == "application/json"
        )
        posted = await aresponses.history[0].request.read()
        assert posted == b'{"tags": ["tag:no-prefix"]}'
